...
static int basicbtfs_iterate(struct file *dir, struct dir_context *ctx) {
    ...
    struct basicbtfs_alloc_table *root_block = NULL;
    ...
    while (block_idx < BASICBTFS_ATABLE_MAX_BLOCKS && root_block->table[block_idx] != 0) {
        ...
        for (; entry_idx < BASICBTFS_ENTRIES_PER_BLOCK; entry_idx++) {
            if (entry->ino && !dir_emit(ctx, entry->hash_name, BASICBTFS_NAME_LENGTH, entry->ino, DT_UNKNOWN)) {
                break;
            }
            ctx->pos++;
            entry++;
...
struct dentry *basicbtfs_search_entry(struct inode *dir, struct dentry *dentry) {
    ...
    struct basicbtfs_inode_info *ci_dir = BASICBTFS_INODE(dir);
    ...
    cblock = (struct basicbtfs_alloc_table *) bh_clusters->b_data;

    while (block_idx < BASICBTFS_ATABLE_MAX_BLOCKS && cblock->table[block_idx] != 0) {
        ...
        for (entry_idx = 0; entry_idx < BASICBTFS_ENTRIES_PER_BLOCK; entry_idx++) {
            if (entry->ino == 0) {
                brelse(bh_block);
                goto lookup_end;
            }
            if (strncmp(entry->hash_name, dentry->d_name.name, BASICBTFS_NAME_LENGTH) == 0) {
                inode = basicbtfs_iget(sb, entry->ino);
                brelse(bh_block);
                goto lookup_end;
            }
            entry++;
...
int basicbtfs_add_entry_name(struct inode *dir, struct inode *inode, char *filename) {
    struct super_block *sb = dir->i_sb;
    struct basicbtfs_inode_info *bfs_dir = BASICBTFS_INODE(dir);
    struct basicbtfs_alloc_table *cblock = NULL;
    struct basicbtfs_entry *entry = NULL;
    ...
    block_idx = BASICBTFS_GET_BLOCK_IDX((cblock->nr_of_entries));
    entry_idx = BASICBTFS_GET_ENTRY_IDX((cblock->nr_of_entries));

    if (cblock->table[block_idx] == 0) {
        bno = get_free_blocks(BASICBTFS_SB(sb), 1);
        if (!bno) {
    ...
    entry = (struct basicbtfs_entry *) bh_dblock->b_data;
    entry += entry_idx;
    entry->ino = inode->i_ino;
    strncpy(entry->hash_name, filename, BASICBTFS_NAME_LENGTH);
...
int basicbtfs_add_entry(struct inode *dir, struct inode *inode, struct dentry *dentry) {
    return basicbtfs_add_entry_name(dir, inode, (char *)dentry->d_name.name);
}
...
int basicbtfs_delete_entry(struct inode *dir, struct inode *inode) {
    ...
    struct basicbtfs_alloc_table *cblock = NULL;
    struct basicbtfs_entry *entry_cur_saddr = NULL,*entry_cur = NULL, *entry_prev = NULL;
    int bi = 0, fi = 0;
    int ret = 0, has_found = false;
    
    bh = sb_bread(sb, BASICBTFS_INODE(dir)->i_bno);
    if (!bh) {
        return -EIO;
    }

    cblock = (struct basicbtfs_alloc_table *) bh->b_data;
    while (bi < BASICBTFS_ATABLE_MAX_BLOCKS && cblock->table[bi] != 0) {
        ...
        entry_cur_saddr = (struct basicbtfs_entry *) bh2->b_data;
        ...
        if (has_found) {
            ...
            for (fi = 0; fi < BASICBTFS_ENTRIES_PER_BLOCK; fi++) {
...
int basicbtfs_update_entry(struct inode *old_dir, struct inode *new_dir, struct dentry *old_dentry, struct dentry *new_dentry, unsigned int flags) {
    ...
    struct basicbtfs_alloc_table *a_table = NULL;
    struct basicbtfs_entry *entry = NULL;
    ...
    a_table = (struct basicbtfs_alloc_table *) bh_new_dir->b_data;

    while (block_idx < BASICBTFS_ATABLE_MAX_BLOCKS && a_table->table[block_idx] != 0) {
        ...
        for (entry_idx = 0; entry_idx < BASICBTFS_ENTRIES_PER_BLOCK; entry_idx++) {
            ...
            if (strncmp(entry->hash_name, new_dentry->d_name.name, BASICBTFS_NAME_LENGTH) == 0) {
                if (flags & (RENAME_NOREPLACE) && new_dir == old_dir) {
                    ...
                    if (new_inode) {
                        ...
                        ret = basicbtfs_delete_entry(new_dir, new_inode);
                    }

                    entry->ino = old_inode->i_ino;
                    strncpy(entry->hash_name, new_dentry->d_name.name, BASICBTFS_NAME_LENGTH);
                    mark_buffer_dirty(bh_block);
                    brelse(bh_block);
                    ret = basicbtfs_delete_entry(old_dir, old_inode);
                    goto end;
                }
            }
            entry++;
    ...
    iterate_end:
    if (a_table->nr_of_entries >= BASICBTFS_ENTRIES_PER_DIR) {
        return -EMLINK;
    }
